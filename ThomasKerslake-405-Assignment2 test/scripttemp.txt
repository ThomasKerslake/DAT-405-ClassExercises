...
// This preload function is for connecting to the api.
function preload() {
  let url = api + sex + "/" + country + "/" + date + "-01-01" + "/" + age + "y/";
  data = loadJSON(url); //Loading the API with a specific query.
}
...
var canvas = createCanvas(1240, 720);
    ...
    var rleData = round(data.remaining_life_expectancy); //Creating a variable to hold the 'remaining_life_expectancy' data from the api.
...
// Grabing and displaying data on header / footer / center.
 ...
 textSize(35);
 fill(255);
 text(data.country.toUpperCase(),-90, 8);
 textSize(14);
 text("RLE: " + round(data.remaining_life_expectancy) + " YEARS",-300, 165); // Remaining life expectancy
 text("AGE: " + age, -300, -157); // Age
 text("DOB: " + date, 230, -157); // Date of birth
 text("SEX: " + data.sex.toUpperCase(), 225, 165); // Sex (male / female)
 pop();
}
